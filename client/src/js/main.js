window.addEventListener('load', async () => {
  ...
  function setContainer() {
      ...
      <aside class="sidebar">
        ...
        <nav role="navigation" class="navbar">
          <a href="/">
            <svg class="home active" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M8 0H1C.4 0 0 .4 0 1v7c0 .6.4 1 1 1h7c.6 0 1-.4 1-1V1c0-.6-.4-1-1-1Zm0 11H1c-.6 0-1 .4-1 1v7c0 .6.4 1 1 1h7c.6 0 1-.4 1-1v-7c0-.6-.4-1-1-1ZM19 0h-7c-.6 0-1 .4-1 1v7c0 .6.4 1 1 1h7c.6 0 1-.4 1-1V1c0-.6-.4-1-1-1Zm0 11h-7c-.6 0-1 .4-1 1v7c0 .6.4 1 1 1h7c.6 0 1-.4 1-1v-7c0-.6-.4-1-1-1Z" fill="#5A698F"/></svg>
          </a>
          <a href="movies">
      ...
      <main>
        ...
        <div role="main" class="contents">
          ...
          <section class="recommended">
            <h3 class="fs-xxl">Recommended for you</h3>
            <div class="cards__r"></div>
          </section>
        </div>
      </main>
    </div>`
  ...
  function setElements(trending, recommend) {
    ...
    function createElement(el, classname = '', value = '', id = null) {
    ...
    function movieInformtion(movie) {
        const movie__details = createElement('div', 'movie__details')
        const top = createElement('div', 'top')
        const year = createElement('p', 'fs-m blend', movie.year)
        ...
        const categoryMovie = createElement('p', 'fs-m blend', movie.category)
        const rating = createElement('p', 'fs-m blend', movie.rating)
        const title = createElement('h3', 'fs-bold-xl title', movie.title)
    ...
    for (const movie of trending) {
        const card__t = createElement('div', 'card__t')
        card__t.append(movieInformtion(movie), thumbnail(movie), bookmark(movie), overlayBg())
        cards.append(card__t)
    }

    // RECOMMEND

    const cards__r = document.querySelector('.cards__r')
    ...
    formSearch.addEventListener('submit', async (e) => {
      e.preventDefault()
      const contents = document.querySelector('.search + div')
  ...
  async function searchAjax(value) {
    const res = await fetch(api + 'search' + `?q=${encodeURI(value)}&type=all`, { method: 'GET' })
    if (res.ok) return res
    return false
  }
