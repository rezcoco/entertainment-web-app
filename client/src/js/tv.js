window.addEventListener('load', async () => {
  ...
  function setElements(tv_series) {
    ...
    for (const movie of tv_series) {
        const cards__s = document.querySelector('.cards__s')
  ...
  async function searchAjax(value) {
    const res = await fetch(api + 'search' + `?q=${encodeURI(value)}&type=tv`, { method: 'GET' })
    if (res.ok) return res
    return false
  }
