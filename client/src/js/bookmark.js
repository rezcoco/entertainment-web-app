(async () => {
  ...
  function setElements(movies, tv) {
    const main = document.querySelector('main')
    const logoutBtn = document.querySelector('.logout__container')
    const avatar = document.querySelector('.avatar')
    const container = document.querySelector('.container__b')
    ...
    if (movies.length > 0) {
        const section_m = createElement('section', 'intl__sect movies__b')
        const h3 = createElement('h3', 'fs-xxl', 'Bookmarked Movies')
        const cards__bm = createElement('div', 'cards__bm')

        section_m.append(h3, cards__bm)
        main.append(section_m)

        for (const movie of movies) {
            const card__r = createElement('div', 'card__r')
            card__r.append(thumbnailR(movie), movieInformtionR(movie))
            cards__bm.append(card__r)
        }
        container.append(section_m)
    }
    if (tv.length > 0) {
        const section_t = createElement('section', 'intl__sect tv__b')
        const h3 = createElement('h3', 'fs-xxl', 'Bookmarked TV Series')
        const cards__bt = createElement('div', 'cards__bt')

        section_t.append(h3, cards__bt)
        main.append(section_t)

        for (const movie of tv) {
            const card__r = createElement('div', 'card__r')
            card__r.setAttribute('id', movie.id)
            card__r.append(thumbnailR(movie), movieInformtionR(movie))
            cards__bt.append(card__r)
        }
        container.append(section_t)
    }
  ...
  async function searchAjax(value) {
    const res = await fetch(api + 'search' + `?q=${encodeURI(value)}&type=bookmark`, { method: 'GET' })
    if (res.ok) return res
    return false
  }
  ...
  async function init() {
      try {
          ...
          if (res.ok) {
              ...
              const movies = bookmarks.filter((movie) => movie.category === 'Movie')
              const tv = bookmarks.filter((movie) => movie.category === 'TV Series')
  
              setContainer()
              return setElements(movies, tv)
          }     
          return refresh(init) 
